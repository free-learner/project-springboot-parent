...
import yhjr.common.rsa.utils.http.CommHttpConstants;
 ...
 *   2017年8月24日
 */
public class MutiHttpClientUtilTest extends BaseServicesTest {
    private static final Logger LOGGER = LoggerFactory.getLogger(MutiHttpClientUtilTest.class);

    @Autowired
    private MutiHttpClientUtil mutiHttpClientUtil;
...
//    @LoadBalanced
    ...
    public void testRestTemplate() {
        ...
        String postParam = JSON.toJSONString(paramMap);
        ResponseEntity<String> postForEntity = restTemplate.postForEntity(url, paramMap, String.class);
        LOGGER.info("http请求結果1為:{}",JSON.toJSONString(postForEntity));
        HttpStatus statusCode = postForEntity.getStatusCode();
        Assert.assertEquals(true,statusCode.is2xxSuccessful());
        String responseBody = postForEntity.getBody();
        LOGGER.info("http请求結果2為:{}",JSON.toJSONString(responseBody));
    }
