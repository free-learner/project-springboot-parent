...
public class IdcardUtils extends StringUtils {  
    ...
    public static void main(String[] args) {
        String idCard="13063419850416313X";
        System.err.println(getGenderByIdCard(idCard));
    }
    
    public static List<String> generateIdCard(String addressCode) {
        List<String> resultList=new ArrayList<>();
        ICNumber ic = null;
        AddressCode addresscodee = new AddressCode(addressCode);
        for (int j = 0; j < 12; j++) {
            ...
            BirthDate birthDate = new BirthDate(formatDate);
            Sex[] sexs={Sex.male,Sex.female};
            for (Sex sex : sexs) {
                for (int i = 100; i < 1000; i++) {
                    SeqCode seqCode = new SeqCode(i+"", sex);
                    if (addresscodee.isValid() && birthDate.isValid() && seqCode.isValid()) {
                        ic = new ICNumber(addresscodee, birthDate, seqCode);
